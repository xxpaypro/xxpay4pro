...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        if( jwtPayload == null ||
                StringUtils.isEmpty(getStringRedisTemplate().opsForValue().get(MchConstant.CACHEKEY_TOKEN_PREFIX_ISV + authToken))
        ){
        ...
        if(jwtBaseUser == null || !jwtPayload.validateToken(jwtBaseUser.getUserId(), jwtBaseUser.getLastPasswordResetTime())){
            getStringRedisTemplate().delete(MchConstant.CACHEKEY_TOKEN_PREFIX_ISV + authToken);
        ...
        if (StringUtils.isNotBlank(jwtPayload.getLoginType()) && (MchConstant.LOGIN_TYPE_APP.equals(jwtPayload.getLoginType()))) {
            getStringRedisTemplate().opsForValue().set(MchConstant.CACHEKEY_TOKEN_PREFIX_ISV + authToken, "1",
