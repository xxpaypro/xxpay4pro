...
public class BaseNotify4MchRefund {
	...
	public JSONObject createNotifyInfo(RefundOrder refundOrder, boolean isFirst) {
		...
		if(isFirst) {
			int result = rpcCommonService.rpcMchNotifyService.insertSelectiveOnDuplicateKeyUpdate(refundOrder.getRefundOrderId(), refundOrder.getMchId(), null, refundOrder.getAppId(), refundOrder.getMchRefundNo(), PayConstant.MCH_NOTIFY_TYPE_REFUND, url);
