...
public class PayOrderController extends BaseController {
    ...
    public String exportExcel() throws Exception {
        ...
        super.writeExcelStream("商户订单列表", excelData);
    ...
    public String exportExcelByAgpay() throws Exception {
        ...
        List<List> excelData = new ArrayList<>();
        List header = Arrays.asList(new String[]{"转账单号", "商户/代理商ID", "发起类型", "账户名", "账号", "代付金额(元)", "手续费(元)", "扣减账户金额", "状态", "代付渠道", "时间"});
        excelData.add(header);
        for(Map record : mchAgentpayRecordList){
            List rowData = new ArrayList<>();
            rowData.add(record.get("TransOrderId"));
            rowData.add(record.get("InfoId"));
            rowData.add((Integer)record.get("InfoType") == MchConstant.INFO_TYPE_MCH ? "商户" : "代理商");
        ...
        super.writeExcelStream("下级代付列表", excelData);
        return null;
    }
