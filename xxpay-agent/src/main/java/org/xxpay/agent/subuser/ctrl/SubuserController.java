package org.xxpay.agent.subuser.ctrl;
...
public class SubuserController extends BaseController {
    ...
    public ResponseEntity<?> get() {
        ...
        SysUser querySysUser = rpcCommonService.rpcSysService.findByUserId(MchConstant.INFO_TYPE_AGENT, getUser().getBelongInfoId(), userId);
    ...
    public ResponseEntity<?> add() {
        ...
        sysUser.setBelongInfoType(MchConstant.INFO_TYPE_AGENT);
        ...
        //判断手机号是否被使用
        SysUser mobileByUser = rpcCommonService.rpcSysService.getOne(
        ...
        //判断邮箱是否被使用
        SysUser emailByUser = rpcCommonService.rpcSysService.getOne(
                new QueryWrapper<SysUser>().lambda()
    ...
    public ResponseEntity<?> update() {
        ...
        SysUser dbRecord = rpcCommonService.rpcSysService.findByUserId(MchConstant.INFO_TYPE_AGENT, getUser().getBelongInfoId(), sysUser.getUserId());
        if(dbRecord == null){
    ...
    public ResponseEntity<?> delete() {
        ...
        rpcCommonService.rpcSysService.batchDelete(MchConstant.INFO_TYPE_AGENT, getUser().getBelongInfoId(), uids);
        return ResponseEntity.ok(BizResponse.buildSuccess());
    }
    ...
    public ResponseEntity<?> viewPermission() {
        ...
        SysUser dbRecord = rpcCommonService.rpcSysService.findByUserId(MchConstant.INFO_TYPE_AGENT, getUser().getBelongInfoId(), userId);
        if(dbRecord == null){
            return ResponseEntity.ok(BizResponse.build(RetEnum.RET_COMM_RECORD_NOT_EXIST));
        }
        ...
        // 得到所有角色
        List<SysRole> sysRoleList = rpcCommonService.rpcSysService.selectAllRole(MchConstant.INFO_TYPE_AGENT, getUser().getBelongInfoId());
