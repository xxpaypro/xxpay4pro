...
import org.xxpay.agent.utils.SpringUtil;
...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        if( jwtPayload == null ||
                StringUtils.isEmpty(getStringRedisTemplate().opsForValue().get(MchConstant.CACHEKEY_TOKEN_PREFIX_AGENT + authToken))
        ){
        ...
        if(jwtBaseUser == null || !jwtPayload.validateToken(jwtBaseUser.getUserId(), jwtBaseUser.getLastPasswordResetTime())){
            getStringRedisTemplate().delete(MchConstant.CACHEKEY_TOKEN_PREFIX_AGENT + authToken);
        ...
        //app端登录   延长token有效期
        if (StringUtils.isNotBlank(jwtPayload.getLoginType()) && ("uni-app".equals(jwtPayload.getLoginType()) || "pc".equals(jwtPayload.getLoginType()))) {
            getStringRedisTemplate().opsForValue().set(MchConstant.CACHEKEY_TOKEN_PREFIX_AGENT + authToken, "1",
