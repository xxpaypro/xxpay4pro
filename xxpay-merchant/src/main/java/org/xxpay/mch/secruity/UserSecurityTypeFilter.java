...
public class UserSecurityTypeFilter extends OncePerRequestFilter {
    ...
    static{
        passUriSet.add("/api/mch/get"); //获取商户信息 否则页面出现错误信息
        passUriSet.add("/api/mch/menu_get"); //获取菜单
//        passUriSet.add("/api/data/orderDayAmount"); //首页统计数据
        passUriSet.add("/api/data/count4dayIncome"); // 首页统计数据
        ...
        passUriSet.add("/api/mch/sms_send"); // 发送验证码
    }
