...
@RestController
@RequestMapping(Constant.MGR_CONTROLLER_ROOT_PATH + "/config/agpay_passage")
@PreAuthorize("hasRole('" + MchConstant.MCH_ROLE_NORMAL + "')")
public class AgpayPassageController extends BaseController {
...
//        if(!getUser().getIsPrivateAccount()) return ResponseEntity.ok(XxPayResponse.build(RetEnum.RET_MCH_PERMISSION_ERROR));
        ...
        agentpayPassage.setBelongInfoId(getUser().getBelongInfoId());
        agentpayPassage.setBelongInfoType(MchConstant.INFO_TYPE_MCH);
...
//        if(!getUser().getIsPrivateAccount()) return ResponseEntity.ok(XxPayResponse.build(RetEnum.RET_MCH_PERMISSION_ERROR));
        ...
        if(agentpayPassage == null || !agentpayPassage.getBelongInfoId().equals(getUser().getBelongInfoId())
           || !agentpayPassage.getBelongInfoType().equals(MchConstant.INFO_TYPE_MCH)) {
...
//        if(!getUser().getIsPrivateAccount()) return ResponseEntity.ok(XxPayResponse.build(RetEnum.RET_MCH_PERMISSION_ERROR));
        ...
        //校验是否越权操作
        AgentpayPassage selectRecord = rpcCommonService.rpcAgentpayPassageService.findById(id);
        if(selectRecord == null || !selectRecord.getBelongInfoId().equals(getUser().getBelongInfoId())
                || !selectRecord.getBelongInfoType().equals(MchConstant.INFO_TYPE_MCH)) {
            return ResponseEntity.ok(XxPayResponse.build(RetEnum.RET_MCH_PERMISSION_ERROR));
        }
...
//        if(!getUser().getIsPrivateAccount()) return ResponseEntity.ok(XxPayResponse.build(RetEnum.RET_MCH_PERMISSION_ERROR));
        ...
        //校验是否越权操作
        AgentpayPassage selectRecord = rpcCommonService.rpcAgentpayPassageService.findById(agentpayPassage.getId());
        if(selectRecord == null || !selectRecord.getBelongInfoId().equals(getUser().getBelongInfoId())
                || !selectRecord.getBelongInfoType().equals(MchConstant.INFO_TYPE_MCH)) {
            return ResponseEntity.ok(XxPayResponse.build(RetEnum.RET_MCH_PERMISSION_ERROR));
        }
