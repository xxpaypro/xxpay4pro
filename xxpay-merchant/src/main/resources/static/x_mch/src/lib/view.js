...
layui.define(['laytpl', 'layer'], function(exports){
  ...
  view.exit = function(){
    view.req({
        type: "GET",
        async: false,
        url: layui.setter.baseUrl + '/logout',
  ...
  view.req = function(options){
    ...
    return $.ajax($.extend({
      ...
      ,success: function(res){
        ...
        else if(res[response.statusName] == statusCode.toSetSecurityPage){
            layer.alert("您还未设置安全认证方式，点击确定跳转到安全中心！", {closeBtn: 0 }
                ,function(aid){
                    location.href = layui.setter.baseLocal + 'account/security/';
                    layer.close(aid);
                });
        }
  ...
  view.popup = function(options){
    ...
    //获取xxpay弹层的参数值
    view.getOpenParams = function(key){
        return typeof($XXPAY_OPEN_PARAMS) != 'undefined' ? $XXPAY_OPEN_PARAMS[key] : null;
    };

    //自定义xxpay弹层
    // popupName: 弹层名称；
    // viewUri: 弹层页面的模块地址， 不包含.html ；
    // openParams: 弹层的传参， 可通过getOpenParams获取；
    // options: 用于layui.open 参数重写， 如关闭按钮等；
    view.xxpayPopup = function(popupName, viewUri, openParams, options){

        $XXPAY_OPEN_PARAMS = openParams; //打开弹层的自定义参数，用于页面传参。
        var viewUrl = views = setter.views + viewUri + setter.engine;
        ...
        $.ajax({
            ...
            ,success: function(html){
                layui.layer.open($.extend({
                    content : "<div class='xxpayPopupDiv'>" + html + "</div>"
                    ,type: 1, title: popupName
                    ,area: ['85%', '90%']
                    ,shade: 0.4, shadeClose: true
                    ,maxmin : true
                    ,btn: ['确认', '取消']
                    ,yes: function(index, layero){
                        layero.find('.xxpayYesBtn').click();  //调用弹层内的绑定元素。
